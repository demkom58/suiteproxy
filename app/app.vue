...
const { systemStatus, health } = useProxyStatus();
...
const authEnabled = computed(() => health.value?.auth_enabled ?? false);
        ...
        <div class="px-5 py-4 border-t border-slate-800/40">
          <div class="flex items-center justify-between">
            ...
            <button
              v-if="authEnabled"
              @click="handleLogout"
              class="text-[10px] text-slate-600 hover:text-slate-400 font-bold uppercase tracking-wider transition-colors"
              title="Sign out"
            >
              Logout
            </button>
