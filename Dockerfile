...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    # Build tools (node-gyp for better-sqlite3)
...
# ── Fix camoufox-js data files ─────────────────────────────────────────
# Nuxt strips non-JS assets from bundled node_modules; copy them back
RUN cp -r node_modules/camoufox-js/dist/data-files \
    .output/server/node_modules/camoufox-js/dist/data-files 2>/dev/null || true
