...
export class PageController {
  ...
  async *pollResponse(abortSignal?: AbortSignal): AsyncGenerator<StreamDelta> {
    ...
    // Debug: log what text was initially detected and check model turn count
    const initialText = await this.getRawResponseText();
    const modelTurnCount = await this.page.evaluate(() =>
      document.querySelectorAll('ms-chat-turn .chat-turn-container.model').length,
    ).catch(() => -1);
    console.log(`[PageController:${this.reqId}] pollResponse: response text detected (${initialText.length} chars, ${modelTurnCount} model turns), starting poll`);
    if (initialText.length > 0 && initialText.length < 500) {
      console.log(`[PageController:${this.reqId}] pollResponse: initial text preview: "${initialText.substring(0, 200)}"`);
    }
    ...
    try {
      for await (const chunk of consumeStreamingChunks(this.page, this.reqId, abortSignal)) {
        ...
        if (chunk.error) {
          ...
          if (!gotAnyData && chunk.error === 'no_data') {
            console.log(`[PageController:${this.reqId}] Network interception didn't fire, falling back to DOM polling`);
            await endCapture(this.page);
            yield* this.pollResponse(abortSignal);
            return;
          }
          // HTTP 403 = permission denied — fall back to DOM polling.
          // This can happen if the external replay AND browser native both fail,
          // or if the replay_fallback_to_browser eventually times out.
          if (!gotAnyData && chunk.error.includes('403')) {
            console.log(`[PageController:${this.reqId}] HTTP 403, falling back to DOM polling`);
            await endCapture(this.page);
            yield* this.pollResponse(abortSignal);
            return;
          }
          // HTTP 429 = rate limit — throw so the retry loop can switch accounts
          if (chunk.error.includes('429')) {
            console.error(`[PageController:${this.reqId}] Rate limit error from replay: ${chunk.error}`);
            throw new RateLimitError(undefined, `Replay returned HTTP 429`, this.reqId);
          }
  ...
  private async waitForResponseText(timeoutMs = 90_000): Promise<void> {
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      // Check for error toasts AND error dialogs (CDK overlay panes)
      try {
        // Error toasts
        ...
        // Error dialogs (Angular Material CDK overlay panes — "Failed to generate content: permission denied")
        const dialogTexts = await this.page.evaluate(() => {
          ...
          overlays.forEach(o => {
            ...
            if (text && text.length > 5) texts.push(text.substring(0, 200));
          });
          return texts;
        }).catch(() => [] as string[]);

        if (dialogTexts.length > 0) {
          const combined = dialogTexts.join(' ').toLowerCase();
          if (combined.includes('rate limit') || combined.includes('quota')) {
            throw new RateLimitError(undefined, dialogTexts[0]);
          }
          if (combined.includes('permission denied') || combined.includes('failed to generate')) {
            throw new Error(`AI Studio error: ${dialogTexts[0]}`);
          }
        }
      } catch (error) {
        if (error instanceof RateLimitError || (
  ...
  private async getRawResponseText(): Promise<string> {
    try {
      ...
      const result = await this.page.evaluate((cmarkSel: string) => {
        ...
        // Fallback: try .model-response-content or innerText of the content area only.
        // IMPORTANT: Do NOT use .model-prompt-container as fallback — it contains
        // UI metadata (conversation titles, timestamps like "Model 6:03 PM", "Greeting")
        // that gets mistaken for actual model output. Only return text from actual
        // content rendering elements.
        const contentEl = responseTurn.querySelector('.response-content, .model-response-text, [data-content-type="model"]');
        if (contentEl) {
          const text = (contentEl as HTMLElement).innerText?.trim() ?? '';
          if (text.length > 0) return { text, error: null };
        }
