...
export function isTier2Recoverable(error: Error): boolean {
  if (error instanceof RateLimitError || error instanceof AuthenticationError || isRateLimitError(error)) {
    return true;
  }
  // Permission denied from Google — typically means temporary account restriction
  const msg = error.message.toLowerCase();
  return msg.includes('permission denied') || msg.includes('caller does not have permission');
}
