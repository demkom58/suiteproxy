...
export async function* enqueueStreamingRequest(
  ...
  try {
    // Retry loop that covers both generator creation AND consumption.
    // Rate limit errors (429) surface during stream consumption (after the generator
    // is created), so executeStreamingWithRetry alone can't catch them.
    for (let streamAttempt = 1; streamAttempt <= MAX_RETRIES; streamAttempt++) {
      let streamResult: AsyncGenerator<StreamDelta>;
      try {
        streamResult = await executeStreamingWithRetry(streamContext, abortSignal);
      } catch (error) {
        // Creation-time errors are already retried by executeStreamingWithRetry.
        // If it throws, all retries failed — propagate.
        throw error;
      }
      ...
      try {
        ...
        // Rate limit errors during stream consumption — switch account and retry
        const err = streamError instanceof Error ? streamError : new Error(String(streamError));
        if (isRateLimitError(err) && streamAttempt < MAX_RETRIES) {
          console.warn(`[Queue] Stream consumption hit rate limit for ${context.reqId} (attempt ${streamAttempt}/${MAX_RETRIES}), switching account...`);
          await handleTier2Recovery(streamContext, err);
    ...
    throw new Error('All stream retry attempts exhausted');
  } finally {
    globalThis._activeRequests = Math.max(0, (globalThis._activeRequests ?? 1) - 1);
    evictIdleSlots().catch(() => {});
  }
...
async function executeRequestViaTextarea(ctx: RequestContext): Promise<QueueResult> {
  ...
  try {
    // If the page was left on a prompt URL from a previous injection request,
    // navigate to new_chat first (injection path skips navigate-back for speed)
    if (!page.url().includes('/prompts/new_chat')) {
...
async function executeRequestViaInjection(ctx: RequestContext): Promise<QueueResult> {
  ...
  try {
    ...
    // Resolve thinking config for Gemini 3.x level-based models
    const thinking = resolveThinking(ctx);

    // Navigate to fake prompt and click Rerun
    // Pass temperature (UI-only param) and thinking level (for Gemini 3.x)
    await triggerGeneration(page, promptId, ctx.reqId, {
    ...
    // Clean up injection routes (navigate-back is deferred — the next injection
    // request will page.goto() its own prompt URL, so navigating to new_chat
    // here is redundant and wastes 2-3s per request)
...
async function executeStreamingRequestViaInjection(
  ...
  try {
    ...
    // Debug: monitor for GenerateContent and key RPC requests
    const genContentHandler = (request: import('playwright-core').Request) => {
      const url = request.url();
      if (url.toLowerCase().includes('generatecontent')) {
        console.log(`[Queue:${ctx.reqId}] >>> GenerateContent DETECTED: ${request.method()} ${url.substring(0, 200)}`);
      } else if (url.includes('MakerSuiteService/') || url.includes('Waa/')) {
        const endpoint = url.split('/').pop()?.split('?')[0] ?? '';
        console.log(`[Queue:${ctx.reqId}] RPC: ${endpoint}`);
      }
    };
    page.on('request', genContentHandler);
    setTimeout(() => { page.off('request', genContentHandler); }, 60_000);
...
async function handleTier2Recovery(_ctx: RequestContext, error: Error): Promise<void> {
  ...
  if (isRateLimitError(error)) {
    ...
    if (accountId) {
      const db = useDb();
      try {
        const limitedUntil = Date.now() + 600_000; // 10 minutes
...
function getAccountForRequest(
  ...
  // Prefer reusing the current account if the browser is already running
  // and the account is not rate-limited. This avoids expensive browser restarts.
  const currentId = getCurrentAccountId();
  if (currentId) {
    const current = db.prepare(`
      SELECT id, creds, fingerprint, proxy FROM accounts
      WHERE id = ? AND limited_until < ?
    `).get(currentId, now) as AccountForRequest | null;
  ...
  const account = db.prepare(`
    ...
    LIMIT 1
  `).get(now) as AccountForRequest | null;

  if (!account) {
    // Fallback: pick any account, even if rate-limited (soonest to expire first)
