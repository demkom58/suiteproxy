 ...
 * - Last turn is always a model placeholder (single space) to trigger rerun
 *
...
export async function buildPromptHistory(
  ...
  // AI Studio needs a final model turn to trigger the Rerun button.
  // Use a single space — it's invisible in the UI but satisfies the
  // non-empty text requirement. Avoid descriptive text like "(placeholder)"
  // because downstream clients may use the assistant's response text for
  // session titling, and we don't want fake text leaking as the title.
  turns.push(createPromptContent('model', ' '));

  // Resolve thinking config based on model type
...
/**
 * Resolve thinking configuration from reasoning_effort.
 *
 * Returns either a numeric budget (for Gemini 2.5) or a named level
 * (for Gemini 3.x) based on model type. The level is used to configure
 * AI Studio's thinking level dropdown via localStorage, while the budget
 * is embedded directly in the PromptHistory's GenerateContentConfig.
 */
export function resolveThinking(ctx: RequestContext): { budget: number | undefined; level: string | undefined } {
  const category = classifyModel(ctx.model);
  const isLevelBased = category === 'THINKING_LEVEL_PRO' || category === 'THINKING_LEVEL_FLASH';
  const isThinkingModel = category !== 'NON_THINKING';

  // Non-thinking models — no config
  if (!isThinkingModel) {
    return { budget: undefined, level: undefined };
  }

  // Explicit disable
  if (ctx.reasoningEffort === 0 || ctx.reasoningEffort === '0') {
    return isLevelBased
      ? { budget: undefined, level: undefined } // Can't disable thinking on level-based models
      : { budget: 0, level: undefined };
  }

  // No budget limit — thinking enabled with no cap
  if (ctx.reasoningEffort === 'none' || ctx.reasoningEffort === -1 || ctx.reasoningEffort === '-1') {
    return isLevelBased
      ? { budget: undefined, level: 'high' }
      : { budget: undefined, level: undefined };
  }

  // Named levels
  if (typeof ctx.reasoningEffort === 'string') {
    const lower = ctx.reasoningEffort.toLowerCase();
    if (['minimal', 'low', 'medium', 'high'].includes(lower)) {
      if (isLevelBased) {
        // Level-based: pass as level name (minimal maps to low for Gemini 3.x)
        const level = lower === 'minimal' ? 'low' : lower;
        return { budget: undefined, level };
      }
      // Budget-based: map level names to approximate budgets
      const budgetMap: Record<string, number> = { minimal: 1024, low: 2048, medium: 8192, high: 32768 };
      return { budget: budgetMap[lower], level: undefined };
    }
  }
  ...
  if (ctx.reasoningEffort !== undefined && ctx.reasoningEffort !== null) {
    const val = typeof ctx.reasoningEffort === 'string'
      ? parseInt(ctx.reasoningEffort as string, 10)
      : ctx.reasoningEffort as number;

    if (typeof val === 'number' && !isNaN(val) && val > 0) {
      if (isLevelBased) {
        // Map numeric budget to level for Gemini 3.x
        const level = val <= 2048 ? 'low' : val <= 8192 ? 'medium' : 'high';
        return { budget: undefined, level };
      }
      return { budget: val, level: undefined };
    }
  }

  // No reasoning_effort specified → apply model-aware defaults
  if (isLevelBased) {
    return { budget: undefined, level: 'high' }; // Default High for Gemini 3.x
  }
  // Gemini 2.5 default
  return { budget: 8192, level: undefined };
}
