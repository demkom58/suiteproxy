...
export async function getPageForSlot(
  ...
  // (skip noisy CSP report-only warnings and AbortErrors from navigation)
  page.on('console', (msg) => {
    if (msg.type() !== 'error') return;
    const text = msg.text();
    if (text.includes('Content-Security-Policy')) return;
    if (text.includes('AbortError')) return;
    console.log(`[BrowserConsole:slot${slot.index}:error] ${text.substring(0, 300)}`);
  });
...
async function launchBrowserAndContext(
  ...
  try {
    ...
    // Per-page network interceptors are installed lazily in prepareNetworkCapture().
    // Context-level interceptor is no longer used (caused cross-request interference
    // when multiple pages shared the same context).
    // The call below is kept for backward compat but is a no-op.
...
export { installInterceptor, installInterceptorOnContext, startCapture, endCapture, consumeChunks, consumeStreamingChunks, hasRouteFired, resetInterceptorState, cleanupPageState } from './network-interceptor';
