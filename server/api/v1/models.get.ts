...
export default defineEventHandler(async (event) => {
  // Don't pass the Authorization header as an account selector — the auth
  // middleware already validated it as a PROXY_API_KEY. The `bearer` parameter
  // in getGoogleApiCredentials is for picking a specific Google account by
  // email, not for API auth. Passing the API key here causes a DB lookup by
  // ID that matches nothing → 503 "No available accounts".
  const creds = getGoogleApiCredentials();
