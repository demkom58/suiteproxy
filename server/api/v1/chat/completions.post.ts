...
export default defineEventHandler(async (event) => {
  ...
  if (body.stream) {
    ...
    // Shared flag — accessible from both start() and cancel() callbacks.
    // When the downstream consumer disconnects (e.g. Caddy timeout, client abort),
    // cancel() fires and sets this flag. The start() callback checks it before
    // every enqueue to avoid "Controller is already closed" errors that would
    // crash Bun as unhandled exceptions.
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const safeClose = (): void => {
          ...
          try {
            ...
            // Already closed by cancel() or prior error
          }
        };

        // Declare keepAlive outside try so it's accessible in finally for cleanup.
        ...
        try {
          ...
          // Send SSE keep-alive comments every 3s while waiting for the first real chunk.
          // SuiteProxy's cold start (browser launch + AI Studio navigation) can take 10-30s.
          // Without keep-alives, reverse proxies (Caddy, Cloudflare) see an idle connection
          // and close it with a 502 before the first chunk arrives.
          // SSE comments (lines starting with ":") are silently ignored by compliant clients.
          let gotFirstChunk = false;
          keepAlive = setInterval(() => {
            if (gotFirstChunk || streamClosed) {
          ...
          for await (const chunk of enqueueStreamingRequest(context)) {
            if (streamClosed) {
              console.log(`[Completions:${reqId}] Stream closed by consumer, stopping`);
              break;
            }
            if (!gotFirstChunk) {
              gotFirstChunk = true;
          ...
          // Send error as SSE event (only if stream is still open)
          const errorChunk = {
            id: reqId,
      ...
      cancel() {
        // Called when the consumer disconnects (e.g. client abort, proxy timeout).
        streamClosed = true;
        console.log(`[Completions:${reqId}] Stream cancelled by consumer`);
        logRequestEnd(reqId, { statusCode: 499, error: 'Client disconnected' });
      },
    });
